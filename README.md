...
| `KEEP_VERSIONS` | `5` | Default number of recent world versions to retain (older blobs pruned after each upload). Adjustable at runtime in the web Admin tools, clamped to 2–10. Valheim's folder-based saves keep no local backups of their own, so these are the only rollback. |
